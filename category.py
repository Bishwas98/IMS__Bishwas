...
class categoryclass:
    def __init__(self,root):
        ...
        self.SupplierTable = ttk.Treeview(Category_Frame,columns=("Category ID","Name"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=Y)
        scrollx.config(command=self.SupplierTable.xview)
        scrollx.config(command=self.SupplierTable.yview)

        self.SupplierTable.heading("Category ID",text="Category ID")
        self.SupplierTable.heading("Name",text="Name")

        self.SupplierTable["show"] = "headings"

        self.SupplierTable.column("Category ID",width=90)
...
#--------------------------------------------------------------------------------------------------
    def add(self):
        ...
        try:
            if self.var_Name.get()=="":
                ...
                cur.execute("Select * from Supplier where Name=?",(self.var_Name.get(),))
                row= cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Category already assigned,try different",parent=self.root)
                else:
                    cur.execute("Insert into Category (Name),value(?)",( self.var_Name.get(),))   
            con.commit()
    ...
    def get_data(self,ev):
        f = self.var_Category_Table.focus()
        content=(self.var_Category_Table.item(f))
        row=content['values']
    ...
    def delete(self):
        ...
        try: 
            if self.var_Category_Id.get()=="":
                ...
                cur.execute("Select * from Category where Category ID=?",(self.var_Supplier_Invoice.get(),))
                row= cur.fetchone()
                if row==None:
                    ...
                    if op==True:
                        cur.execute("Delete from Category where Category ID=?",(self.var_Category_Id.get(),))
                        con.commit()
